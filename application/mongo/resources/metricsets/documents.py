...
class StandardMetricSetMetadata(BaseMetadata):
...
class MongoStandardMetricSetConfig(MongoResourceConfig):
    ...
    def get_by_uri(cls, uri: str):
        return MongoStandardMetricSetConfig.objects(uri=uri).first()
    ...
    def create(cls, data, context: UserWorkspaceResourceContext, save: bool = True):
        ...
        if not result:
            ...
            obj = cls(
                ...
                owner=owner,
                workspace=workspace,
                metadata=StandardMetricSetMetadata(created=now, last_modified=now),
            )
            if save:
    ...
    def validate_input(cls, data, context: ResourceContext) -> TBoolStr:
        ...
        if not all(fname in data for fname in required):
            ...
            config = MongoBuildConfig.get_by_name(data['build'])    # TODO If is None?
